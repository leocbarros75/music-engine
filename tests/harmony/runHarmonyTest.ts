...
import { EXPECTATIONS_BY_BASENAME, type HarmonyOutput } from "./expectations";
...
function findBeat(out: HarmonyOutput, measure: number, beat: number) {
  const beats = out.beats ?? [];
  return beats.find((b) => b.measureNumber === measure && b.beatNumber === beat) ?? null;
}

function findMeasure(out: HarmonyOutput, measure: number) {
  const measures = out.measures ?? [];
  return measures.find((m) => m.measureNumber === measure) ?? null;
}

function hasCadenceType(out: HarmonyOutput, atMeasure: number, type: string): boolean {
  const cadences = out.cadences ?? [];
  return cadences.some((c) => c.atMeasure === atMeasure && c.type === type);
}

function hasCadenceEvidence(out: HarmonyOutput, atMeasure: number, prevRoman: string, lastRoman: string): boolean {
  const cadences = out.cadences ?? [];
  return cadences.some(
    (c) =>
      c.atMeasure === atMeasure &&
      (c.evidence?.prevRoman ?? "") === prevRoman &&
      (c.evidence?.lastRoman ?? "") === lastRoman
  );
}

function hasWarning(out: HarmonyOutput, atMeasure: number, type: string, atBeat?: number): boolean {
  const warnings = out.warnings ?? [];
  return warnings.some((w) => {
    if (w.atMeasure !== atMeasure) return false;
    if ((w.type ?? "") !== type) return false;
    if (typeof atBeat === "number") return w.atBeat === atBeat;
    return true;
  });
}
...
if (!exp) {
  die(
    `ASSERTION FAILED: No expectations found for ${base}. Add it to tests/harmony/expectations.ts (EXPECTATIONS_BY_BASENAME).`
  );
}
...
for (const ce of exp.cadenceEvidence ?? []) {
  assert(
    hasCadenceEvidence(out, ce.atMeasure, ce.prevRoman, ce.lastRoman),
    `Expected cadence evidence at m${ce.atMeasure}: ${ce.prevRoman} -> ${ce.lastRoman}.`
  );
}

for (const w of exp.warnings ?? []) {
  assert(
    hasWarning(out, w.atMeasure, w.type, w.atBeat),
    `Expected warning type ${w.type} at measure ${w.atMeasure}${typeof w.atBeat === "number" ? ` beat ${w.atBeat}` : ""}.`
  );
}
