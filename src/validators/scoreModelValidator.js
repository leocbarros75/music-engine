    ...
    for (const part of score.parts) {
        for (const m of part.measures) {
            ...
            for (const ev of m.events) {
                ...
                if (ev.staff === 1) {
                    const cur = byT[t].top;
                    if (!cur || midi > cur.midi)
                        byT[t].top = { midi, id: ev.id };
                }
                // LH staff=2: pick lowest note at time t
                if (ev.staff === 2) {
                    const cur = byT[t].bot;
                    if (!cur || midi < cur.midi)
                        byT[t].bot = { midi, id: ev.id };
                }
            }
...
export function validateScoreModelPiano(score) {
    const issues = [];
    ...
    for (let i = 0; i < outer.length; i++) {
        const cur = outer[i];
        if (cur.topMidi == null || cur.botMidi == null)
            continue;
        ...
        if (prev.topMidi == null || prev.botMidi == null)
            continue;
...
//# sourceMappingURL=scoreModelValidator.js.map