// src/analyze/melodyDetector.ts
import type { ScoreModel } from "../score/types";

type NoteEvent = any;

export function detectSimpleMelody(score: ScoreModel) {
  // Minimal: choose the highest staff-1 note at each onset
  const melodyIds = new Set<string>();

  for (const part of score.parts) {
    for (const m of part.measures) {
      const byT: Record<number, NoteEvent[]> = {};
