 ...
 */
// (worship-only forceBassRoot / colour-enrichment helpers intentionally omitted)
...
export function arrangeStringEnsemble(
  ...
  // SYMPHONIC: deliberately NO worship-idiom harmony post-processing here.
  //   • no forceBassRoot — a Romantic bass line moves: inversions, passing
  //     tones and pedal points are the point, not a root on every chord.
  //   • no add9/m7 triad enrichment — Brahms writes a plain triad when he
  //     wants a plain triad; colour comes from the chord symbol, not from us.
  // The DP's own voice-leading result stands. (The low-interval spacing limit
  // still applies later in the arranger — that is acoustics, not style.)
