...
type JazzBandOptions = {
  ...
  // Piano comping controls (RH only)
  pianoAddBeat4?: boolean;
  pianoAddAnticipation?: boolean; // "and of 4" (beat 4 + eighth)
...
const STYLE_PRESETS: Record<JazzStyle, StylePreset> = {
  swing: {
    style: "swing",
    pianoAddBeat4: true,
...
function resolveOptions(opts?: JazzBandOptions): StylePreset {
  const style: JazzStyle = opts?.style ?? "swing";
...
function pickChordMidiNotes(ch: any): number[] {
  const notes = (ch?.notes ?? []).slice().sort((a: any, b: any) => (a.midi ?? 0) - (b.midi ?? 0));
  return notes.map((n: any) => n.midi).filter((m: any) => typeof m === "number");
}

function chooseChordForTime(chordsInMeasure: any[], targetT: number): any | null {
  if (!chordsInMeasure || chordsInMeasure.length === 0) return null;

  const sorted = chordsInMeasure.slice().sort((a: any, b: any) => (a.t ?? 0) - (b.t ?? 0));

  // Prefer the last chord at or before targetT
  let best: any | null = null;
  for (const ch of sorted) {
    const t = ch.t ?? 0;
    if (t <= targetT) best = ch;
    if (t > targetT) break;
  }
  return best ?? sorted[0] ?? null;
}

function unique2(a: number, b: number): [number, number | null] {
  if (a === b) return [b, null];
  return a <= b ? [a, b] : [b, a];
}

/**
 * Canonical kit ids (exporter recognizes these).
 */
type DrumKind = "ride" | "hihat_closed" | "hihat_open" | "kick" | "snare" | "crash";

function drumId(kind: DrumKind): string {
  return kind;
}
...
function writeDrumsForStyle(
  ...
  const addAtBeat = (beatIndex0: number, kind: DrumKind) => {
    const t = beatIndex0 * beatDur;
    if (t >= 0 && t < barDur) addUnpitched(measure, t, hitDur, drumId(kind), 1, 1);
  };

  const addAtOffset = (t: number, kind: DrumKind, durOverride?: number) => {
    if (t >= 0 && t < barDur) addUnpitched(measure, t, durOverride ?? hitDur, drumId(kind), 1, 1);
  };
  ...
  if (preset.style === "swing") {
    // Ride quarters
    for (let b = 0; b < beats; b++) addAtBeat(b, "ride");

    // Hi-hat on 2 and 4
    if (beats >= 2) addAtBeat(1, "hihat_closed");
    if (beats >= 4) addAtBeat(3, "hihat_closed");
  ...
  if (preset.style === "bossa") {
    ...
    addAtBeat(0, "kick");
    if (beats >= 3) addAtBeat(2, "kick");

    if (beats >= 2) addAtBeat(1, "snare");
    if (beats >= 4) addAtBeat(3, "snare");

    for (let b = 0; b < beats; b++) addAtBeat(b, "hihat_closed");
  ...
  // ballad
  {
    ...
    addAtBeat(0, "kick");
    addAtBeat(0, "ride");
    if (beats >= 3) addAtBeat(2, "ride");
    if (beats >= 3) addAtBeat(2, "hihat_closed");
...
export function mapPianoToJazzBandOpen(score: ScoreModel, options?: JazzBandOptions): ScoreModel {
  ...
  for (const ch of chords) {
    ...
    if (mTBN < mBAS) mTBN = shiftOctavesToward(mTBN + 12, TBN_LO, TBN_HI, C_TBN);
    if (mTSX < mTBN) mTSX = shiftOctavesToward(mTSX + 12, TSX_LO, TSX_HI, C_TSX);
    if (mASX < mTSX) mASX = shiftOctavesToward(mASX + 12, ASX_LO, ASX_HI, C_ASX);
    if (mTPT < mASX) mTPT = shiftOctavesToward(mTPT + 12, TPT_LO, TPT_HI, C_TPT);
