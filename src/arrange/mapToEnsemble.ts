 ...
 */
export function mapPianoToFullOrchestraBasic(score: ScoreModel): ScoreModel {
  const srcPart = score.parts[0];
  const chords = extractOnsetChords(score);
  ...
  for (const p of partsOut) partById[p.part_id] = p;

  // Create measure shells for every part
  const measureShellsByPartAndNum: Record<string, Record<string, any>> = {};
  for (const p of partsOut) measureShellsByPartAndNum[p.part_id] = {};
  ...
  // Helpers
  const pickRange = (instrument: string) => {
    const key = instrument as keyof typeof InstrumentCatalog;
    const found = (InstrumentCatalog as any)[key];
    if (found) return found;
    // fallback to a safe violin-like range if unknown
    return { midi_low: 55, midi_high: 103 };
  };

  const putNote = (partId: string, measureNum: number, t: number, dur: number, midi: number, voice: number) => {
 ...
 */
export function mapToEnsemble(score: ScoreModel, ensemble: string): ScoreModel {
  if (ensemble === "string_quartet") return mapPianoToStringQuartetOpen(score);
  if (ensemble === "full_orchestra") return mapPianoToFullOrchestraBasic(score);
