...
function addUnpitched(measure, t, dur, instrumentId, voice = 1, staff = 1) {
...
function addPitched(measure, t, dur, pitch, voice = 1, staff = 1) {
...
function writeDrumsSwing(measure, divisions, beats, opt) {
    ...
    if (opt.enableSwingSkip) {
        ...
        if (t1 > 0 && t1 < barDur)
            addUnpitched(measure, t1, hitDur, "ride", 1, 1);
        // after beat 3 (in 4/4: beat index 2)
        if (beats >= 3) {
            const t3 = 2 * beatDur + swingOffset;
            if (t3 > 0 && t3 < barDur)
                addUnpitched(measure, t3, hitDur, "ride", 1, 1);
        }
    }
    // Backbeat hats and/or snare on 2 and 4
    if (opt.includeHiHatBackbeat) {
        if (beats >= 2)
            addUnpitched(measure, 1 * beatDur, hitDur, "hihat", 1, 1);
        if (beats >= 4)
            addUnpitched(measure, 3 * beatDur, hitDur, "hihat", 1, 1);
    }
    if (opt.includeSnareBackbeat) {
        if (beats >= 2)
            addUnpitched(measure, 1 * beatDur, hitDur, "snare", 1, 1);
        if (beats >= 4)
            addUnpitched(measure, 3 * beatDur, hitDur, "snare", 1, 1);
    }
    // Light kick on 1
    addUnpitched(measure, 0, hitDur, "kick", 1, 1);
}
function writeDrumsBossa(measure, divisions, beats, opt) {
    const beatDur = divisions;
    const barDur = beats * beatDur;
    const hitDur = Math.max(Math.floor(divisions / opt.hitDurFraction), 1);
    // Light hat on each beat
    for (let b = 0; b < beats; b++)
        addUnpitched(measure, b * beatDur, hitDur, "hihat", 1, 1);
    // Kick on 1 and 3
    addUnpitched(measure, 0, hitDur, "kick", 1, 1);
    if (beats >= 3)
        addUnpitched(measure, 2 * beatDur, hitDur, "kick", 1, 1);
    ...
    if (tAnd2 > 0 && tAnd2 < barDur)
        addUnpitched(measure, tAnd2, hitDur, "hihat", 1, 1);
    if (tAnd4 > 0 && tAnd4 < barDur)
        addUnpitched(measure, tAnd4, hitDur, "hihat", 1, 1);
}
function writeDrumsBallad(measure, divisions, beats, opt) {
    const beatDur = divisions;
    const barDur = beats * beatDur;
    const hitDur = Math.max(Math.floor(divisions / opt.hitDurFraction), 1);
    ...
    if (beats >= 3)
        addUnpitched(measure, 2 * beatDur, hitDur, "ride", 1, 1);
    // Keep bar readable if nothing else lands
    if ((measure.events?.length ?? 0) === 0)
        addRest(measure, 0, barDur, 1, 1);
}
function writeDrumsByStyle(measure, divisions, beats, opt) {
    if (opt.style === "bossa")
        return writeDrumsBossa(measure, divisions, beats, opt);
    if (opt.style === "ballad")
        return writeDrumsBallad(measure, divisions, beats, opt);
    return writeDrumsSwing(measure, divisions, beats, opt);
}
function writeColorHits(drumsMeasure, divisions, beats, chordOnsets, opt) {
    const beatDur = divisions;
    const barDur = beats * beatDur;
    const hitDur = Math.max(Math.floor(divisions / opt.hitDurFraction), 1);
    ...
    const pickT = (i) => unique[i] ?? fallbackTs[i] ?? beatDur;
    if (opt.enableBells)
        addUnpitched(drumsMeasure, pickT(0), hitDur, "bells", 1, 1);
    if (opt.enableChimes)
        addUnpitched(drumsMeasure, pickT(1), hitDur, "chimes", 1, 1);
    if (opt.enableMallets)
        addUnpitched(drumsMeasure, pickT(2), hitDur, "mallets", 1, 1);
...
function writeTimpani(timMeasure, divisions, beats, chordOnsets, opt) {
    const beatDur = divisions;
    const barDur = beats * beatDur;
    ...
    const snapToBeat = (t) => Math.round(t / beatDur) * beatDur;
 ...
 */
export function mapToPercussion(score, options) {
    ...
    for (const m of srcPart.measures) {
        ...
        const sortedHits = hits.slice().sort((a, b) => (a.t ?? 0) - (b.t ?? 0));
        // Groove for DRUMS
        ...
        if (opt.kickOnChordOnsets) {
            if (sortedHits.length > 0) {
                ...
                for (const ch of sortedHits) {
                    ...
                    if (t >= barDur)
                        continue;
                    const tooClose = (drmShell.events ?? []).some((ev) => {
                        if (ev?.type !== "unpitched")
                            return false;
                        if (ev?.instrumentId !== "kick")
                            return false;
        ...
        // Colors on DRUMS
        const chordOnsets = sortedHits.map((x) => ({ t: x?.t ?? 0 }));
        ...
        if ((drmShell.events?.length ?? 0) === 0)
            addRest(drmShell, 0, barDur, 1, 1);
    }
    return {
...
export function mapPianoToPercussionOpen(score, options) {
    return mapToPercussion(score, options);
}
//# sourceMappingURL=mapToPercussion.js.map