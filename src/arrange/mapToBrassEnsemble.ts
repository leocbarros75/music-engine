...
export function mapPianoToBrassEnsembleOpen(score: ScoreModel): ScoreModel {
  ...
  for (const ch of chords) {
    ...
    if (mTB < mTU) mTB = shiftOctavesToward(mTB + 12, rTB.midi_low, rTB.midi_high, CENTER_TBN_WR);
    if (mHN < mTB) mHN = shiftOctavesToward(mHN + 12, rHN.midi_low, rHN.midi_high, CENTER_HORN_WR);
    if (mT2 < mHN) mT2 = shiftOctavesToward(mT2 + 12, rT2.midi_low, rT2.midi_high, CENTER_TPT2_WR);
    if (mT1 < mT2) mT1 = shiftOctavesToward(mT1 + 12, rT1.midi_low, rT1.midi_high, CENTER_TPT1_WR);
