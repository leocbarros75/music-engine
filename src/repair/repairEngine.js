...
function getKeyFifthsFromScore(score) {
...
function buildMajorScalePcsFromFifths(fifths) {
...
function findFirstMeasureWithRHNote(score) {
    for (const part of score.parts) {
        for (const m of part.measures) {
            const rh = m.events.filter(e => e.type === "note" && e.staff === 1);
            if (rh.length > 0)
                return { part, measure: m, rh };
        }
    }
    return null;
}
function shiftSingleRHNoteAt(score, measureNumber, t, direction) {
    let changed = false;
    for (const part of score.parts) {
        for (const m of part.measures) {
            ...
            for (const ev of m.events) {
                ...
                if (p.alter === undefined)
                    delete ev.pitch.alter;
                else
                    ev.pitch.alter = p.alter;
...
function shiftSingleRHNoteAtDiatonic(score, measureNumber, t, direction, scalePcs) {
    let changed = false;
    for (const part of score.parts) {
        for (const m of part.measures) {
            if (m.number !== measureNumber)
...
function addLHSupportFromRH(score) {
    ...
    measure.events.sort((a, b) => a.t - b.t);
    return true;
}
function shiftLHDownOctave(score) {
    let changed = false;
    for (const part of score.parts) {
        for (const m of part.measures) {
            for (const ev of m.events) {
...
function shiftRHByStep(score, direction) {
...
export function repairScoreModel(scoreModel, issues) {
    // Clone scoreModel to avoid mutating caller
    const score = JSON.parse(JSON.stringify(scoreModel));
    const applied = [];
    ...
    if (hasPar8 || hasPar5) {
        const offending = issues.find(i => i.type === "parallel_8ves_outer_voices" || i.type === "parallel_5ths_outer_voices");
    ...
    if (hasDirect8 || hasDirect5) {
        const offending = issues.find(i => i.type === "direct_8ves_outer_voices" ||
