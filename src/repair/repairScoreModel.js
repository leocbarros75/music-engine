...
function findEventById(score, id) {
...
function diatonicStepUp(midi, keyFifths) {
...
export function repairScoreModel(scoreModel, issues) {
    ...
    for (const issue of issues ?? []) {
        if (issue.severity !== "error")
            continue;
        if (!fixTypes.has(issue.type))
            continue;
        const loc = issue.location ?? {};
        const topId = loc.top_event_id;
        if (!topId)
            continue;
        const topEv = findEventById(repaired, topId);
        if (!topEv || topEv.type !== "note")
            continue;
        const keyFifths = repaired.parts?.[0]?.measures?.[0]?.attributes?.key_fifths ??
            repaired.parts?.[0]?.measures?.[0]?.attributes?.key_fifths ??
            0;
        ...
        if (p.alter == null)
            delete topEv.pitch.alter;
        else
            topEv.pitch.alter = p.alter;
        applied.push({
...
//# sourceMappingURL=repairScoreModel.js.map