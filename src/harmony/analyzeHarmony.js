...
function keyFromScoreModelKeySignature(score, hist) {
    ...
    function fit(tonicPc, rel) {
        const allowed = new Set(rel.map((d) => normPc(tonicPc + d)));
        let inside = 0;
        let outside = 0;
        ...
        if (domPc !== null) {
            ...
            if (bpc === null || normPc(bpc) !== domPc) {
                warnings.push({
                    ...
                    message: `Skipped cadential 6/4 relabel at measure ${m}: bassPc is not dominant for key ${String(key?.tonic ?? "?")}.`,
        ...
        if (domPc !== null) {
            ...
            if (bpc === null || normPc(bpc) !== domPc) {
                warnings.push({
                    ...
                    type: "cadential64_skipped",
                    message: `Skipped cadential 6/4 relabel at measure ${Number(cur?.measureNumber ?? i + 1)}: bassPc is not dominant for key ${String(key?.tonic ?? "?")}.`,
                    expected: `bassPc=${domPc}`,
                    found: bpc === null ? "bassPc=null" : `bassPc=${normPc(bpc)}`
                });
                continue;
...
function trackSecondaryResolutionsBeatwise(beats, key, warnings) {
    const isNC = (x) => looksLikeNC(String(x?.roman?.roman ?? ""));
...
function anchorKeyToFinalTriadIfNeeded(params) {
    ...
    if (curConf >= 0.98)
        return key;
    let lastTriadRoot = null;
    let lastTriadQuality = null;
    ...
    if (lastTriadRoot === null || lastTriadQuality === null)
        return key;
    let firstTriadRoot = null;
    let firstTriadQuality = null;
...
export function analyzeHarmony(req) {
    try {
        ...
        let key = null;
        const hadForceKey = !!(options.forceKey?.tonic && (options.forceKey.mode === "major" || options.forceKey.mode === "minor"));
        ...
        if (granularity === "measure") {
            ...
            const cadences = detectCadences(measures);
            return {
                ok: true,
        ...
        const cadences = detectCadences(measureSnapshots);
        return {
            ok: true,
...
//# sourceMappingURL=analyzeHarmony.js.map