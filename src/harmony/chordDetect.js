...
export function detectChordFromPcs(pcsIn, preferSharps = true, bassPc) {
    const pcs = uniqSorted(pcsIn);
    const bpc = bassPc === null || bassPc === undefined ? null : (((bassPc % 12) + 12) % 12);
    if (pcs.length === 0)
        return { pcs, rootPc: null, bassPc: bpc, quality: "unknown", name: "N.C." };
    ...
    for (const r of pcs) {
        ...
        if (!best || score > best.score)
            best = { root: r, score, quality: qFinal };
    }
...
//# sourceMappingURL=chordDetect.js.map