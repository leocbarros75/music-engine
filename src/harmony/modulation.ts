...
function sameKey(a: KeyLike, b: KeyLike): boolean {
  return a.tonic === b.tonic && a.mode === b.mode;
}

function histAdd(hist: number[], pcs: number[]): void {
  for (const pc of pcs) {
    const i = ((pc % 12) + 12) % 12;
    hist[i] = (hist[i] ?? 0) + 1;
  }
...
function histForWindow(getMeasurePcs: (mi: number) => number[], startMi: number, endMi: number): number[] {
  const hist = new Array(12).fill(0);
  for (let i = startMi; i <= endMi; i++) histAdd(hist, getMeasurePcs(i));
  return hist;
}
...
export function detectKeyChanges(params: {
  ...
  minConfidence?: number; // default 0.82
}): KeyChange[] {
  ...
  if (measureCount <= windowSize) return [];
  ...
  for (let start = 0; start <= measureCount - windowSize; start++) {
    ...
    const guess = keyFromMetaOrBestGuess(null, hist, true) as KeyLike;

    if (!guess?.tonic || (guess.mode !== "major" && guess.mode !== "minor")) continue;
    if (typeof guess.confidence !== "number") continue;
