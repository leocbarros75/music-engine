          ...
          ? (() => {
              ...
              const profile = (usePolyphonic
                ...
                : styleRaw === "baroque"
                  ? "bach_chorale"
                  : (settings.stringTexture ?? "melody_harmony")) as ProfileId;
              const stringResult = arrangeStringEnsemble(scoreModel, chords, { profile, sustainAccompaniment: true, keepInnerVoicesBelowMelody: true });
              warnings.push(...(stringResult.warnings ?? []));
              const stringScore = stringResult.scoreModel;
              if (usePolyphonic) {
      ...
      ? useStringEnsembleArranger
        ? (() => {
            ...
            const stringResult = usePolyphonic
              ...
              : arrangeStringEnsemble(scoreModel, chords, { profile, sustainAccompaniment: true, keepInnerVoicesBelowMelody: true });
            warnings.push(...(stringResult.warnings ?? []));
            const stringScore = stringResult.scoreModel;
            if (usePolyphonic) {
