...
# Run parallels check automatically (prints and continues)
npx tsx scripts/checkSatbParallels.ts ./tmp/satb_response.json || true

# Run cadence check automatically (prints warnings only)
npx tsx scripts/checkCadences.ts ./tmp/satb_response.json || true

# Run voicing integrity check (no missing inner voices)
npx tsx scripts/checkSatbVoicingIntegrity.ts ./tmp/satb_response.json || true
