...
function shouldMatchKeyName(k: string): boolean {
...
function findPaths(
  ...
  hits: { path: (string | number)[]; key: string; value: unknown }[],
  maxHits: number
): void {
...
function findDirectFifths(node: unknown, pathSoFar: (string | number)[], hits: { path: (string | number)[]; value: unknown }[], maxHits: number): void {
  if (hits.length >= maxHits) return;
...
function printMeasureAttributeSummary(score: any): void {
...
function main(): void {
  ...
  const hits: { path: (string | number)[]; key: string; value: unknown }[] = [];
  ...
  const fifthsHits: { path: (string | number)[]; value: unknown }[] = [];
